...
void splitSliceYZ(const char* input_file,
                  int tpc,
                  const char* plane,
                  int z_bin_index)
{
    ...
    double z_min = binsZ[z_bin_index];
    double z_max = binsZ[z_bin_index + 1];
...
int main(int argc, char** argv)
{
    if (argc < 4) {
        std::cout << "Usage:\n"
                  << "./produceSliceYZ input.root TPC plane [z_bin_index]\n";
        return 1;
    }
    ...
    splitSliceYZ(input_file, tpc, plane, z_bin_index);
    return 0;
}