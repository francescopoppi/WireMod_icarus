...
{
    if (argc < 4) {
        std::cout << "Usage: ./ntupleAnalyzer [Offbeam|Overlay] [binned|unbinned|both] [fileListPath]\n";
        return 1;
    }
    ...
    {
        std::ofstream runningFile("running.txt", std::ios::trunc);
        if (!runningFile.is_open()) {
            std::cerr << "ERROR: Cannot write to running.txt\n";
            return 1;
        }
        runningFile << fileListPath << "\n";
        runningFile.close();
    }
    ...
    auto runUnbinnedWrapper = [&](const std::vector<std::string>& files) {
        struct HitVars {
        ...
        for (const auto& f : files) {
            ...
            auto West = reader.readFile(file,1,isOverlay);
            auto East = reader.readFile(file,0,isOverlay);
            file->Close();
    ...
    auto runBinnedWrapper = [&](const std::vector<std::string>& files) {
        HistogramManager hm;
        TrackReader reader(corrections);
    ...
    if (mode == "unbinned") {
        ...
        runUnbinnedWrapper(files);
    }
    else if (mode == "binned") {
        std::cout << "Running BINNED mode\n";
        runBinnedWrapper(files);
    }
    else if (mode == "both") {
        std::cout << "Running UNBINNED mode\n";
        runUnbinnedWrapper(files);
